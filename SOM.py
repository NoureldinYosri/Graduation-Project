"""
    code for the self organizing map is borrowed from SACHIN JOGLEKAR'S tutorial
    link : https://codesachin.wordpress.com/2015/11/28/self-organizing-maps-with-googles-tensorflow/
    and updated by me : Noureldin Yosri
"""
...
def train(path,m = 5,n = 5,niter = 100):
    """
    ...
    def get_all_image_paths(path):
        ...
        for (dirpath, dirnames, filenames) in walk(path):
            image_paths.extend(map(lambda filename:path + "/" + filename,filenames));
            nxt = map(lambda dirname:path + "/" + dirname,dirnames);
            break
        for sub_path in nxt:
            get_all_image_paths(sub_path);
    get_all_image_paths(path);
    image_paths = filter(lambda path: path.endswith('.jpg'),image_paths);
#    image_paths = random.sample(image_paths,100);

    retSOM = SOM(m,n,64,n_iterations = niter);
    ...
    for image_path in image_paths:
        ...
        for feature_description in des:
            x = np.array(feature_description);
            x = np.reshape(x,(1,64));
            retSOM.train(x);
